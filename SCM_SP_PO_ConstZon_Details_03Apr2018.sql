 use EIP
 go
 --   SQLSCM.#SCM_SP_PO_ConstZon_Details '07-May-2018', '13-May-2018', 1, 523
...
INSERT INTO #Temp_SCM_PO_Details
                        ...
                        Temp_Currency_Code, Temp_Material_Code, Temp_PO_Basic_Rate,Temp_PO_Qty, Temp_PO_Value,
						...
						Temp_Created_By, Temp_Reason_for_Higher_Price)
	SELECT DISTINCT  HPO_PO_Number, HPO_Last_Amendment_Number,
										HPO_PO_Date, HPO_Job_Code, HPO_BA_Code, HPO_Currency_Code, DPO_Material_Code, DPO_Basic_Rate,DPO_Qty,DPO_Value,
										...
										DPOCD_National_Low_Rate, MMAT_UOM_Code, HPO_Inserted_By, DPOCD_Remarks
	FROM SQLSCM.SCM_H_Purchase_Orders, SQLSCM.SCM_D_Purchase_Orders, SQLMAS.GEN_M_Materials, 
					SQLSCM.SCM_D_Counter_Offer, SQLSCM.SCM_D_PO_ConstZon_Details
	WHERE HPO_PO_Number = DPO_PO_Number AND HPO_PO_Date >= @dtFromDate AND HPO_PO_Date <= @dtToDate
					...
					AND HPO_Company_Code = @intCompanyCode 	 
...
INSERT INTO #Temp_SCM_PO_Details
		...
		SELECT DISTINCT HPOAR_Request_Number, HPO_Last_Amendment_Number+1,
				 HPOAR_Request_Date, HPOAR_Job_Code, HPO_BA_Code, HPOAR_Currency_Code, DPOAR_Material_Code, DPOAR_Basic_Rate,DPOAR_Qty,DPOAR_Value,
				DCOFF_Basic_Rate, CASE WHEN DPOAC_Rate_Applicable IN ('Y','R') THEN 'Y' ELSE 'N' END, 
				   ...
				   DPOAC_National_Low_Rate,MMAT_UOM_Code, HPOAR_Inserted_By, DPOAC_Remarks
		FROM SQLSCM.SCM_H_Purchase_Orders, SQLSCM.SCM_H_PO_Amend_Request, SQLSCM.SCM_D_PO_Amend_Request,
					SQLSCM.SCM_D_Counter_Offer, SQLSCM.SCM_D_POA_ConstZon_Details,SQLMAS.GEN_M_Materials 
		WHERE HPO_PO_Number = HPOAR_PO_Number AND HPOAR_Request_Number = DPOAR_Request_Number 
					AND HPOAR_Request_Date >= @dtFromDate AND HPO_PO_Date <= @dtToDate
					...
					AND MMAT_Company_Code= @intCompanyCode AND HPO_Company_Code = @intCompanyCode 
...
SELECT Temp_PO_Number PO_Number,Temp_PO_Amendment_No PO_Amendment_No, Temp_PO_Date PO_Date, 
						...
						Temp_Location_Desc Location_Desc ,						
						 Temp_Material_Code Material_Code,  replace( replace( replace( replace( replace( replace( replace( replace(replace(MMAT_Material_Description,char(9),'-'),char(10),'-'),char(11),'-'),char(12),'-'),char(13),'-'),char(14),'-'),'''','-'),'"','-'),',','-') Material_Name, 
						...
						Temp_PO_Basic_Rate PO_Material_Basic_Rate, 	Temp_PO_Qty PO_Material_Qty,
						...
						MEMP_Employee_ID Creator_PSNo , MEMP_Name Created_By  , Temp_Reason_for_Higher_Price Reason_for_Higher_Price
